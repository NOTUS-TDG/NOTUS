  ...
  return (
    <div className="space-y-6">
      ...
      {selecionado && disciplinas.estado === "ok" && (
        <section key={selecionado.userId} className="space-y-3">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <h3 className="font-display text-xl font-bold text-foreground">
              Boletim de {selecionado.studentName}
            </h3>
            <span className="text-sm text-muted-foreground">Matrícula {selecionado.matricula}</span>
          </div>
          <p className="text-sm text-muted-foreground">
            Clique em uma nota para ver, editar ou lançar avaliações. A média geral de cada período
            é a média das médias das matérias.
          </p>
          <BoletimMatriz
