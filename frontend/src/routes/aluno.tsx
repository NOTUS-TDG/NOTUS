...
function Presenca() {
  ...
  return (
    <Secao
      ...
      descricao={geral !== null ? `Presença geral: ${geral}% · mínimo exigido 75%` : "Calculada a partir das aulas e faltas registradas pelo professor."}
    >
      <Card>
        <CardContent className="space-y-5 p-6">
          <Button onClick={buscar} disabled={carregando} variant="outline" className="min-h-11 text-base">
            {carregando ? "Atualizando..." : "Atualizar"}
          </Button>
          {erro && <p className="text-base font-semibold text-destructive">{erro}</p>}
          {!erro && frequencia === null && <p className="text-base text-muted-foreground">Carregando…</p>}
          {frequencia && frequencia.length === 0 && (
            <p className="text-base text-muted-foreground">
              Nenhuma aula registrada ainda pelo professor — a presença aparece aqui assim que a chamada começar.
            </p>
          )}
          {frequencia?.map((f) => {
...
function FaltasBackend() {
  ...
  return (
    <Secao titulo="Faltas registradas" descricao="Suas faltas lançadas pelo professor, vindas do backend (GET /falta/me).">
      <Card>
